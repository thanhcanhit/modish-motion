...
document.addEventListener("DOMContentLoaded", async () => {
	...
	async function renderCartItems() {
		...
		for (const item of cart.items) {
			...
			if (item.quantity > item.variant.availableQuantity) {
				...
				showToast(
					`Số lượng sản phẩm "${item.variant.name}" đã được điều chỉnh do vượt quá số lượng có sẵn`,
					"warning"
				);
			...
			// Thêm tổng tiền cho từng sản phẩm
			const itemTotal = document.createElement("p");
			const total = item.variant.price * item.quantity;
			itemTotal.textContent = `Tổng: ${total.toLocaleString()} đ`;
			itemTotal.classList.add("text-gray-900", "font-semibold", "mt-1");
	...
	async function updateTotalPrice() {
		...
		// Update progress bar và shipping fee
		...
		// Cập nhật thông báo và phí vận chuyển
		let shippingFee = SHIPPING_FEE;
		if (progressValue >= 100) {
			freeShippingMessageElement.innerHTML =
				'<b class="font-semibold text-success">Bạn đã được miễn phí vận chuyển!</b>';
